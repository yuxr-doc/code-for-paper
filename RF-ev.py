...
#---------------------
def calculate_average_shap(shap_records_forEV):
    """
    计算所有外折的平均 SHAP 值

    参数:
    shap_records_forEV -- 列表, 包含元组 (fold, shap_values_matrix)
        shap_values_matrix: 二维数组, 形状为 (n_samples, n_features)

    返回:
    average_shap_matrix -- 平均 SHAP 值矩阵, 形状 (n_samples, n_features)
    """
    # 如果记录为空，返回空数组
    if not shap_records_forEV:
        return np.array([])

    # 获取样本数量和特征数量
    _, first_shap_matrix = shap_records_forEV[0]
    n_samples = first_shap_matrix.shape[0]
    n_features = first_shap_matrix.shape[1]

    # 初始化累加矩阵
    total_shap = np.zeros((n_samples, n_features))
    fold_count = 0

    # 累加所有外折的 SHAP 值
    for fold, shap_matrix in shap_records_forEV:
        # 验证形状一致性
        if shap_matrix.shape != (n_samples, n_features):
            raise ValueError(f"折 {fold} 的 SHAP 矩阵形状 {shap_matrix.shape} "
                             f"与第一折的形状 {(n_samples, n_features)} 不匹配")

        total_shap += shap_matrix
        fold_count += 1

    # 计算平均值
    average_shap_matrix = total_shap / fold_count

    return average_shap_matrix
...
def process_scores(scores):
    ...
    column_mapping = {
        ...
        'best_params_min_samples_split': 'Best_min_samples_split',
        'best_params_min_samples_leaf': 'Best_min_samples_leaf',
        'best_params_max_features': 'Best_max_features',
        'best_params_class_weight': 'Best_class_weight'
    }
...
# 初始化SHAP值收集容器-外部队列
shap_records_forEV = []  # 使用字典存储样本索引与SHAP值集合
...
for fold, (out_train_index, out_valid_index) in enumerate(outer_cv.split(X_raw, Y_raw)):
    ...
    study.optimize(lambda trial: objective(trial, X_train_raw, y_train_raw), n_trials=100,
                    show_progress_bar=True)  # 进行 100 次试验
    ...
    shap_values = explainer.shap_values(X_val_scaled)[:,:,1]  # 获取SHAP值矩阵
    ...
    # 外部队列的shap值计算
    explainer_forEV = shap.TreeExplainer(best_model)
    shap_values_forEV = explainer.shap_values(X_val_scaled_ev)[:,:,1]  # 获取SHAP值矩阵
    shap_records_forEV.append((fold, shap_values_forEV))
    ...
    print(f"外部验证 AUC: {ev_auc:.3f} | 外部验证 f1: {ev_f1:.3f}| 外部验证 accuracy: {ev_accuracy:.3f}| 外部验证 recall: {ev_recall:.3f}| 外部验证 precision: {ev_precision:.3f}")
    ##################################################
    ...
    print(f"Model saved: {json.dumps(result, indent=2)}")
    '''
# 转换为数组
...
# 外部验证集的shap后处理：构建矩阵
shap_matrix_forEV = calculate_average_shap(shap_records_forEV)
...
# 导出到Excel
excel_path = "rf_model_performance.xlsx"
...
#shap.summary_plot(shap_matrix_forEV, X_raw_ev, feature_names=feature_names, max_display=37)
shap.summary_plot(shap_matrix_forEV, X_raw_ev, feature_names=feature_names, plot_type='bar', max_display=37)
...
scaler_forshow = StandardScaler()
X_raw_forshow = scaler_forshow.fit_transform(X_raw_ev)
shap_explanation = shap.Explanation(
values=shap_matrix_forEV, # SHAP值矩阵
data=X_raw_forshow, # 原始特征数据
feature_names=feature_names # 特征名称
)
# 配置全局绘图参数（符合学术出版标准）
...
fig.set_constrained_layout_pads(w_pad=0.08)
# 保存多种格式（满足期刊要求）
plt.savefig('SHAP_Analysis-ev.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('SHAP_Analysis-ev.pdf')  # PDF用于排版
plt.savefig('SHAP_Analysis-ev.svg')  # SVG用于后期编辑
plt.show()
'''
#-----------------------
# 训练集 输出forceplot
...
feature_name0 = "RTEMP_Acc_SpecEn"
feature_name1 = "RFH_Ang_AbsEnergy"
feature_name2 = "RFH_Ang_Mean"
feature_name3 = "RFH_Ang_ApEn"
shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name0)],   color='#6086d1',    show=False)
# 保存多种格式（满足期刊要求）
plt.savefig('0dependence_plot.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('0dependence_plot.svg')  # SVG用于后期编辑

shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name1)],   color='#6086d1',    show=False)
plt.savefig('1dependence_plot.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('1dependence_plot.svg')  # SVG用于后期编辑

shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name2)],   color='#6086d1',    show=False)
# 保存多种格式（满足期刊要求）
plt.savefig('2dependence_plot.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('2dependence_plot.svg')  # SVG用于后期编辑
# 保存多种格式（满足期刊要求）
plt.savefig('3dependence_plot.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('3dependence_plot.svg')  # SVG用于后期编辑

#独立验证集输出forceplot
shap_explanation = Explanation(
    values=shap_matrix_forEV,
    data=X_raw_ev,
    feature_names=feature_names
)
feature_name0 = "RTEMP_Acc_SpecEn"
...
shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name0)],   color='#6086d1',    show=False)
# 保存多种格式（满足期刊要求）
plt.savefig('0dependence_plot-ev.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('0dependence_plot-ev.svg')  # SVG用于后期编辑

shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name1)],   color='#6086d1',    show=False)
plt.savefig('1dependence_plot-ev.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('1dependence_plot-ev.svg')  # SVG用于后期编辑

shap.plots.scatter(shap_explanation[:, feature_names.index(feature_name2)],   color='#6086d1',    show=False)
# 保存多种格式（满足期刊要求）
plt.savefig('2dependence_plot-ev.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('2dependence_plot-ev.svg')  # SVG用于后期编辑
# 保存多种格式（满足期刊要求）
plt.savefig('3dependence_plot-ev.tif',  # TIFF用于投稿
           dpi=600,
           pil_kwargs={"compression": "tiff_lzw"})
plt.savefig('3dependence_plot-ev.svg')  # SVG用于后期编辑
'''
##################################################################################################################

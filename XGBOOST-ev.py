...
def objective(trial, x_train, y_train):
    ...
    param = {
        'learning_rate': trial.suggest_float('learning_rate', 0.001, 0.2, log=True),  # 学习率
        'max_depth': trial.suggest_int('max_depth', 3, 6),  # 最大深度
        #'min_child_weight': trial.suggest_int('min_child_weight', 5, 20),
        'subsample': trial.suggest_float('subsample', 0.6, 0.9),  # 子样本比例
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.7, 0.9),  # 特征采样比例
        'n_estimators': trial.suggest_int('n_estimators', 100, 500),  # 树的数量
        #'reg_lambda': trial.suggest_float('reg_lambda', 1e-3, 5, log=True),  # L2正则化项
        'reg_alpha': trial.suggest_float('reg_alpha', 1e-3, 1.0, log=True),# L1正则化项
        'gamma': trial.suggest_float('gamma', 0.1, 1),
        #'scale_pos_weight': trial.suggest_float('scale_pos_weight',max(1, scale_pos_weight*0.97),min(10, scale_pos_weight*1.03))
    }
    metrics = {
        'accuracy': [],
        'f1': [],
    ...
    for train_index, valid_index in inner_cv.split(x_train, y_train):
        ...
        x_train_scaled_infold = scaler_IN.fit_transform(x_train_infold)
        ...
        # 转换验证集数据（不可参与任何拟合）
        x_invalid_infold = scaler_IN.transform(x_invalid_infold)  # 使用训练集的scaler
...
for fold, (out_train_index, out_valid_index) in enumerate(outer_cv.split(X_raw, Y_raw)):
    ...
    X_train_scaled = scaler_OUT.fit_transform(X_train_raw)
    X_val_scaled = scaler_OUT.transform(X_val_raw)
    ...
    #ev_X_train_scaled = scaler_EV.fit_transform(X_raw)
    #best_model.fit(ev_X_train_scaled, Y_raw,
                   # eval_set=[(X_train_scaled, y_train_raw)],verbose=False
#                   )
    ##################################################
    # 外部验证的数据标准化
    X_val_scaled_ev = scaler_OUT.transform(X_raw_ev)
    y_val_ev = Y_raw_ev
    ##################################################
    ##################################################
    # 外部验证的性能指标
    ...
    shap_values = explainer(X_val_scaled).values  # 获取SHAP值矩阵
...
# 导出到Excel
excel_path = "xgb_model_performance.xlsx"
...
feature_name1 = "RFH_Ang_Mean"
feature_name2 = "RTEMP_Ang_ApEn"
